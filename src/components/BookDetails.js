import { useState, useEffect } from "react";
import { Link, useParams } from "react-router-dom";
import { get } from "../BooksAPI";

const BookDetails = () => {
  const { id } = useParams();
  const [book, setBook] = useState(undefined);
  const options = {
    currentlyReading: "Currently Reading",
    wantToRead: "Want to Read",
    read: "Read",
  };

  useEffect(() => {
    document.title = "Book Detials for ID " + id;
    const getBooksRes = async () => {
      const b = await get(id);
      if (b) {
        if (!b.error) {
          setBook(b);
          document.title = b.title;
        } else {
          setBook(null);
          document.title = "Book Detials for ID " & id;
        }
      } else {
        setBook(null);
        document.title = "Book Detials for ID " & id;
      }
    };
    getBooksRes();
  }, [id]);

  const thumbnail =
    book !== undefined
      ? book.imageLinks
        ? book.imageLinks.thumbnail
        : `data:image/jpeg;base64,`
      : "";
  const title = book !== undefined ? book.title : "";
  const authors =
    book !== undefined
      ? book.authors
        ? book.authors.length > 1
          ? book.authors.join(", ")
          : book.authors[0]
        : ""
      : "";
  const shelf = book !== undefined ? options[book.shelf] : "";
  const subtitle = book !== undefined ? book.subtitle : "";
  const description = book !== undefined ? book.description : "";
  const publishedDate = book !== undefined ? book.publishedDate : "";
  const publisher = book !== undefined ? book.publisher : "";
  const pageCount = book !== undefined ? book.pageCount : "";
  const averageRating = book !== undefined ? book.averageRating : "";
  const ratingsCount = book !== undefined ? book.ratingsCount : "";
  const language = book !== undefined ? book.language : "";

  return (
    <div className="list-books">
      <div className="list-books-title">
        <h1>MyReads</h1>
      </div>
      <div className="list-books-content">
        {book !== undefined ? (
          <div className="book-content">
            <h2 className="book-content-title">
              {title} {shelf && <span>- Shelf {shelf}</span>}
            </h2>
            <div className="book-content-details">
              <img src={thumbnail} className="book-content-cover" alt={title} />
              <ul>
                <li key="authers">
                  <span>Authers</span> : {authors}
                </li>
                <li key="subtitle">
                  <span>Subtitle</span> : {subtitle}
                </li>
                <li key="description">
                  <span>Description</span> : {description}
                </li>
                <li key="publisher">
                  <span>Publisher</span> : {publisher}
                </li>
                <li key="publishedDate">
                  <span>PublishedDate</span> : {publishedDate}
                </li>
                <li key="language">
                  <span>Language</span> : {language}
                </li>
                <li key="pageCount">
                  <span>PageCount</span> : {pageCount}
                </li>
                <li key="ratings">
                  <span>Ratings</span> : {averageRating} Stars / {ratingsCount}{" "}
                  Rates
                </li>
              </ul>
            </div>
          </div>
        ) : (
          <div className="book-content">
            <h2 className="book-content-title">Loading Book Details ...</h2>
          </div>
        )}
      </div>
      <div className="back-tomain">
        <Link to="/">Back to main page</Link>
      </div>
    </div>
  );
};

export default BookDetails;
